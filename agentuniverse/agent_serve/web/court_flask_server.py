...
from agentuniverse.agent_serve.service_instance import ServiceInstance, ServiceNotFoundError
from agentuniverse.agent_serve.web.request_task import RequestTask
from agentuniverse.agent_serve.web.web_util import make_standard_response, request_param, service_run_queue
...
def service_run(service_id: str, params: dict, saved: bool = False):
    ...
    results = request_task.run()

    def generate():
        for result in results:
            with app.app_context():
                LOGGER.debug(f"request_task.run() {result}")
                response = make_standard_response(success=True, result=result, request_id=request_task.request_id)
