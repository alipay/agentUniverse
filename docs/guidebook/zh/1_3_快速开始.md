...
### 应用工程准备
我们提供了一个标准的工程模板，您可以到 [这里](https://gitee.com/agentUniverse/agentUniverse-Guides/blob/master/sample_standard_app) 进行下载。您也可以通过以下命令下载：
```shell    
...
在`xx_agent_case_b.yaml`中，我们定义了一个`DemoPeerAgent`的配置。`info`部分是agent的基本信息设置，`plan`部分包含了agent的行为规划设置(决定了agent的工作模式)，`metadata`部分包含了agent对象的元数据设置。
...
### 启动服务
通过在IDE中执行app/bootstrap/server_application.py文件
