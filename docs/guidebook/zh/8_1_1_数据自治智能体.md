...
在我们开放的mvp版本中，你可以通过`dataset_build_agent`智能体对于待评测的问题集进行多轮次的批量调用，生产评测数据集；之后通过`dataset_eval_agent`智能体采用蚂蚁公布的Fin-Eva数据评测标准基于7个维度（相关性、事实性、合理性、时效性、结构性、整体性、全面性）进行智能体数据评估和标注，将多轮次评测结果进行丰富的统计学分析，以提供给用户更直观的Agent能力变化。

通过一整套mvp机制，您可以非常轻松的通过智能化能力清晰的了解您的智能体能力当前处于什么水平。

需要特别说明的是，整个评测报告得分为参考值，在实际投产过程中，我们将多轮多次得分进行综合对比，以分辨智能体效果好坏。
...
如下所示，建立两条问题集，input对应智能体的query信息：
```jsonl
{"input": "英伟达股票大涨的原因是什么？"}
{"input": "贵州茅台为什么不行了？"}
```
...
[dataAgent生产的评测数据集样例地址](../../../sample_standard_app/app/examples/data/dataset_turn_1_2024-07-09-20:46:11.jsonl)
...
如下图所示: 
 ...
 - Overall Score: 综合分数 = 多个维度的评估分数总和 / 维度个数
 - Query: 评测数据问题
 - Answer: 评测数据答案
 - 相关性 Score: 代表评测数据答案与问题的相关相关性，分数越高越相关
 - 相关性 Suggestion: 代表相关性维度存在的问题和改进建议
 - 更多维度 Score/Suggestion 以此类推
![data_agent_eval_result](../_picture/data_agent_eval_result.png)

[dataAgent生产的完整评测结果样例地址](../../../sample_standard_app/app/examples/data/eval_result_turn_1_2024-07-09-20:46:11.xlsx)
...
如下图所示: 
 ...
 - Overall Avg Score: 单轮数据集中所有数据的综合分数总和 / 数据集中对应数据量
 - 相关性 Avg Score: 单轮数据集中所有数据的相关性分数总和 / 数据集中对应数据量
 - 更多维度 Avg Score 以此类推
![data_agent_eval_report](../_picture/data_agent_eval_report.png)

[dataAgent生产的综合评测报告样例地址](../../../sample_standard_app/app/examples/data/eval_report_2024-07-09-20:46:11.xlsx)

### 对比实验
调整aU中`demo_rag_agent`中的模型从上文生产评测报告时的**qwen1.5-72b-chat**改为**qwen1.5-7b-chat**，通过dataAgent评测后，生成的综合评测报告如下:
![data_agent_eval_report](../_picture/data_agent_eval_report_qwen7b.png)
对比两次综合评测报告，可以看到各维度多轮评分均存在大幅度下降，以分辨模型变更后，智能体效果差异。
