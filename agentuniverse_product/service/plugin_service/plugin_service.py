...
from agentuniverse_product.service.model.plugin_dto import OPENAPISPluginDTO, PluginDTO
...
from agentuniverse_product.service.util.plugin_util import assemble_plugin_product_config_data, parse_openapi_to_tool_input, parse_openapi_yaml_to_tool_bundle, validate_create_plugin_parameters
...
class PluginService:
        ...
        for product in product_list:
            if product.type == 'PLUGIN':
                plugin_dto = PluginDTO(nickname=product.nickname, avatar=product.avatar, id=product.id,
                                       description=product.description,openapi_desc = product.openapi_desc,toolset=product.toolset)
    ...
    @staticmethod
    def transform_to_openapi_plugin(plugin_dto: OPENAPISPluginDTO) -> str:
        pass

    @staticmethod
    def create_plugin_with_openapi(plugin_dto: OPENAPISPluginDTO) -> str:
        # 一个 openapi schema 有多个 api ，解析成多个 tool_bundles，
