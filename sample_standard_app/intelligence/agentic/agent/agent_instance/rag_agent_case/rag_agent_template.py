...
class RagAgentTemplate(AgentTemplate):
    ...
    def customized_execute(self, input_object: InputObject, agent_input: dict, memory: Memory, llm: LLM, prompt: Prompt,
        ...
        agent:Agent = AgentManager().get_instance_obj('law_rag_agent')
        res = agent.run(input=agent_input['input'])

        law_aget_output = res.get_data('output')

        rag_agent = AgentManager().get_instance_obj('demo_rag_agent')

        res = rag_agent.run(input=agent_input['input'])

        rag_agent_output = res.get_data('output')

        agent_input['background'] = (law_aget_output + "\n\n" + rag_agent_output)
